...
# Makes the script run both with and without kernprof
try:
    profile
except NameError:
    def profile(func):
...
def jacobi(u, interior_mask, max_iter=20_000, atol=1e-4):
    """
    Reference-style Jacobi solver using NumPy.
    """
    u = np.copy(u)
...
def main():
    parser = argparse.ArgumentParser(
        description="Parallel wall-heating simulation with static scheduling over floorplans."
    )
    ...
    # Avoid launching more workers than there are buildings
    ...
    # Flatten results while preserving static chunk order
